...
SimulationManager::simulate(){
    ...
    while(!eventq.empty() || !sched->empty()){
        ...
        switch(evt->get_state()){
            ...
            case TRANS_TO_RUN:
            {
                ...
                if(proc->get_remaining_exec_time() > 0){
                    ...
                    if(sched->get_quantum() != numeric_limits<int>::max()){
        ...
        if (CALL_SCHEDULER){
            ...
            Event* new_evt = new Event(CURRENT_RUNNING_PROCESS, TRANS_TO_RUN, CURRENT_TIME);
...
SimulationManager::get_next_event(){
    ...
    stable_sort(eventq.begin(), eventq.end(), OrderEvents());
...
SimulationManager::calc_io_util(){
    stable_sort(interval_arr.begin(), interval_arr.end(), OrderIntervals());
...
SimulationManager::print_all(){
    ...
    stable_sort(finished_procs.begin(), finished_procs.end(), OrderProcs());
...
SimulationManager::get_next_rand(int burst)
{
    int idx = 1 + (rand_vals[rand_offset] % burst);
    rand_offset++;
    if(rand_offset >= num_rands)
        rand_offset = 0;
    return idx;
